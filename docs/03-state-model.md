...
> ✅ **`calibration acceptable (held-out)` is enforced** (S-1012, OQ-9/DL-042).
> `gate1_status()` requires `calibration_ok`, computed by
> `models/metrics.py::hypo_calibration` on `P(state ≤ 2)`: three risk bands, judged only at
> ≥ 20 predictions each, gap ≤ **0.10 when it understates** risk and ≤ **0.20 when it
> overstates** — asymmetric because understating is the direction that costs her a warning.
> No qualifying band ⇒ **not acceptable** (cannot judge means do not open).
> **All five conditions in the diagram are now implemented.** This box previously claimed
> that after S-1007 — wrongly; DL-041 corrected it and S-1012 made it true.
