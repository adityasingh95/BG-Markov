...
| **REQ-007** (daily Tresiba → `basal_log`) | S-201 (schema) · **S-1013 (capture — NOT BUILT)** | `test_all_tables_present…` (basal_log) — schema only | ⚠️ **SCHEMA ONLY — NO CAPTURE SURFACE.** Corrected 2026-07-18 (**DL-046**): this row previously read *"✅ Schema done (form: S-302/EPIC 3)"*, but S-302 delivered bolus **timing** and EPIC 3 closed without a basal form. **Nothing in the system can write a basal dose** — a grep for `BasalLog` returns only its own definition. `effective_basal` is a model feature (REQ-021), so **the model cannot be fitted on real data until S-1013 lands**; it is a hard blocker for S-1009. Unnoticed because every consumer so far either tested the EWMA in isolation (S-402, pure) or fabricated the value in memory (demo, E2E cycle) — **a feature nobody has sourced end-to-end is a feature nobody has checked exists.** |
