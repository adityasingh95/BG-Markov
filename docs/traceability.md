...
| **REQ-045/059 + INV-2/INV-9** (a logged meal produces a logged, unseen prediction) | **S-1015 [SAFETY]** | `tests/safety/test_live_prediction.py` (17) — ★ INV-2 asserted **twice**: on the response body *and* on the `MealCreated` schema by AST, because a body check alone passes if a prediction field exists but is null; ★ the readout still shows her nothing **with a real prediction in the database** — a test that could not be written before this story; ★ a plain model failure is swallowed and her meal survives, **and** a `SafetyViolation` still propagates (the two must not be conflated); ★ an out-of-range **baseline** is caught, the meal is kept and the refusal is **recorded** (DL-053 amended); ★ a refusal is **not scored** as a prediction; ★ served features compared element-wise against the trained ones; no promoted model / an **unpromoted** one / no profile each mean no row and no error | ✅ **Done 2026-07-30 (DL-053, DL-054).** Four plants, all caught. `prediction_log` fills and the 90-day clock starts for the first time — verified on the seeded 240-day DB: 6 meals logged over the real API, 6 predictions recorded, clock at 4 days, response keys still exactly `meal_id`/`test_at`/`message`. |
| **REQ-001/002/006** (`05 §` idempotency on the meal endpoint) | **S-1016 [SAFETY]** | `tests/safety/test_meal_idempotency.py` (9) — ★ the headline assertion is on **`bolus_log`, not the meal count**: an implementation that dedupes the meal and re-inserts the boluses passes a meal-count test and preserves the entire harm; then ★ **IOB** via `iob_at_start_at`, then ★ **the recommended dose** — the end of the causal chain from a double-tap to an under-dose; the **database itself** refuses a duplicate key (a check-then-insert races); same key + different data leaves the first record unchanged and audits nothing; two different keys with identical data create two meals; several historical NULL keys coexist | ✅ **Done 2026-07-30 (DL-055).** Four plants, all caught. `idempotency_key` had been **required and read by nothing** since S-301 — the fourth declared-and-unconnected gap. |
| **REQ-004/005 + ADR-8** (a reported clinical time is naive local; an offset is refused) | **S-1018 [SAFETY]** | `tests/unit/test_naive_timestamps.py` (7) — ★ the sharpest is `test_it_raises_rather_than_converting`: `value.replace(tzinfo=None)` returns a naive datetime, passes `mypy --strict`, and returns **02:30 for an 08:00 breakfast**, so asserting *"the result is naive"* passes on it; `+00:00` is pinned separately because a zero offset is still an offset and its accidental correctness in UTC is what hid this. `tests/integration/test_naive_clinical_times.py` (12) — 422 **not 500** on all five reported fields, the naive-still-works regressions, and ★ an explicit test that `logged_at` is **not** covered. `tests/e2e/test_timezone.py` (3) — ★ **the test that would have caught it**: in an `Asia/Kolkata` browser, 08:00 typed ⇒ 08:00 stored (it stored 02:30); the row's `meal_type` and hour must agree (it said `breakfast` at 02:30); the post-BG reading saves from a non-UTC phone. `tests/forbidden/test_client_timestamp_patterns.py` (6) — no `new Date(x).toISOString()` in client JS, proven to bite on the causing line **and** the split-across-lines evasion, and proven **not** to fire on the correct form | ✅ **Done 2026-07-30 (DL-057).** Two plants, both caught. Found by `scripts/demo_ui.py` — the first run of the UI in a real browser. |
| **REQ-001/002/006/013** (idempotency reaches the **client**; corrections too) | **S-1019** | `tests/e2e/test_double_tap.py` (5) — ★ two taps ⇒ one meal **and one bolus**; ★ two separate entries ⇒ **two** meals (the over-eager fix loses her second breakfast, which is the worse failure — a duplicate is visible in the data, an absence is not); ★ **a dropped response** (`route.fetch()` then `route.abort()`) makes the retry case testable independently of the disabled button; the restored draft; the button disabled in flight, observed by MutationObserver rather than a sleep. `tests/integration/test_correction_idempotency.py` (6) — repeat ⇒ one event **and one bolus**, first record unchanged, DB-level constraint, NULL coexistence, and ★ an assertion that post-BG/basal/profile deliberately have **no** key. `tests/forbidden/test_idempotency_key_minting.py` (4) | ✅ **Done 2026-07-30 (DL-058).** Three plants; ★ the first was **not** caught by the browser tests as first written — see S-1021. |
| **REQ-007/054/061** (every form that posts to `/api/` actually submits) | **S-1020** | `tests/forbidden/test_native_form_post.py` (5 + parametrised over every template) — ★ the guard for the **class**: any `<form action="/api/…">` with no script **in its own template**; `base.html`'s shared helpers deliberately do not count; bespoke templates exempted **by name** with a companion test that each still loads its named handler. `tests/e2e/test_operator_forms.py` (4) — the basal row exists **and the browser did not navigate away**; a repeat date corrects rather than duplicates (S-1013, now reachable); a new profile version; ★ the out-of-range flag is **shown** (DL-048's other half) | ✅ **Done 2026-07-30 (DL-059).** ★ The guard found a **third** form nobody had reported — the **revoke / kill switch** on `/operator/shadow`. Raised, not fixed: there is **no promote control at all**. |
| **REQ-001/010/011** (a browser test asserts on the row, not on the toast) | **S-1021** | `tests/e2e/conftest.py` — `assert_saved`, `db_session` on the server's own database, `browser_at(tz)`. `tests/e2e/test_assert_saved.py` (13) — ★ the assertion that the assertion works, incl. the empty toast the obvious weakening lets through, and a two-way drift guard on the success/failure copy. `tests/e2e/test_post_bg_flow.py` rewritten to assert `post_bg` and `elapsed_min` | ✅ **Done 2026-07-30 (DL-056).** ★ `test_post_bg_form_asks_reported_time_and_saves` had been **green since S-303 on the toast "Could not save — please try again."** |
| **REQ-001/040 + `05b §2`** (the design system; selection state is visible, and not by colour alone) | **S-1022** | `tests/a11y/test_visual_state.py` (9) — ★ an unselected favourite renders **no tick** (all three showed one) and the chosen one does, checked through the **box model** because a tick hidden with `display:none` is still in `textContent`; ★ pressed and unpressed differ in a **non-colour** property, since a design that signals selection by fill alone is invisible in greyscale — the exact failure `05b §2` names; one-per-group for favourites, timing presets and the before/after pair; the primary action is **wider than a chip**, so hierarchy is asserted not intended; ★ body-text contrast measured in **both** colour schemes (dark was **1.9:1**). Every assertion is on COMPUTED STYLE — asserting on `app.css` source proves a rule was typed, not that it applies | ✅ **Done 2026-07-30 (DL-060).** Two plants, both caught. ★ Six layout defects found by running it, four of them the same shape: a hard floor in `rem`, fine on a laptop, sideways on her phone at 200%. |
| **REQ-057 / `09 §2.1`** (a browser run leaves a video and a trace) | **S-1023** | `tests/e2e/test_recording.py` (3) — ★ every assertion is on the **file**, not on the option being set: *configured* and *produced* have already diverged twice here (S-1015, S-1016). Recording **off** leaves no `.webm` and no `.zip` and `page.video` is `None` — the reverse guard, so the default cannot drift into the slow one; recording **on** writes a named video and a named trace, both non-empty; the trace carries a `.trace` stream **and** screenshots, because a trace that opens to an empty timeline looks like evidence and answers nothing; ★ **only the named video survives** — `save_as` copies, and the first real run left four videos for two contexts | ✅ **Done 2026-07-31 (DL-061).** ★ The stray-video defect was missed by the three tests written first, because each asserted *the named file exists* — true whether or not the unnamed one also does. **The assertion that caught it is about absence.** |
| **REQ-056 / `06 §7`** (local deployment; a synthetic database says so) | **S-1024 [SAFETY]** | `tests/safety/test_demo_banner.py` (20) — ★ **every** HTML GET route, enumerated from the app by its declared `response_class`, not hand-listed, plus a guard that the enumeration is non-empty or every one of them passes vacuously; ★ **no** page shows it on an unmarked database (an always-on banner would sit on her real records calling them fake); ★ exactly **one** `Jinja2Templates` under `api/`; ★ `seed_demo_db()` always marks what it makes. `tests/unit/test_provenance.py` (5) — absence means *not marked*, idempotent re-mark, survives a reopen | ✅ **Done 2026-07-31 (DL-062).** Two plants, both caught; the second by the structural guard **and** the page test. ★ The first implementation read a **different database than the page** — worse than no banner, because a mislabelled page is trusted. |
| **REQ-059/060** (a promoted artifact can actually serve) | **S-1014** | `tests/integration/test_model_store.py` (13) — ★ round trip asserted on **probabilities** to 1e-9, not on parameters (a double-applied scaler leaves every stored number identical and every probability wrong — measured, not assumed); ★ fit on SCALED features and predict on RAW rows, which is the only place a missing/ignored/doubled scaler shows; ★ an AST guard that no `pickle`/`dill`/`joblib`/`marshal` is imported under `models`/`data`/`prescribe`/`api`; ★ a pre-S-1014 artifact loads as `None`, not an exception — old rows are real history; a mismatched feature ORDER is refused (the coefficient vector is positional); the refit stores a model on the **same representation it scored** | ✅ **Done 2026-07-30 (DL-052).** Three plants, all caught. Migration verified on the pre-existing 240-day demo DB; a reloaded model serves P(low) spanning 0.0004-0.9986 over 319 real meals. |
| **REQ-060** (monthly refit → new unpromoted artifact) | **S-1009** | `tests/integration/test_refit.py` (24) + `test_refit_surfaces.py` (12) — ★ unpromoted asserted on a DB that **already has a promoted artifact**, so "inherits promotion" fails loudly instead of passing vacuously; a refit appends and never overwrites; ★ the assembler goes through `get_training_set` so INV-7 stays wired, asserted by the **ledger reconciliation** (the only assertion a private query cannot satisfy); ★ `effective_basal` is the S-402 EWMA and **not** the raw daily dose, asserted on a **step-change** fixture so the forbidden shortcut cannot pass by coincidence; the trailing-183-day boundary checked **both sides**; ★ no composite weight is ever zero, over a worst-case oldest/lowest-confidence row; a 90-day-old meal weighs exactly half; ★ hypo recall is `null` not `0.0` when a window contains no lows; `cli refit` says "NOT PROMOTED" in words | ✅ **Done 2026-07-30 (DL-047, DL-049, DL-050).** Four plants; three died, **one survived and exposed a real gap** — see the story outcome. |
...
- **EPIC 10 — Integration, UI & End-to-End Validation — BACKLOG (not started, DL-033).**
  ...
  - **★ Review addendum (2026-07-18, DL-034) — operational-spine stories added.** Holding
    ...
    - **REQ-060 → S-1009 (monthly refit cadence) — ✅ Done 2026-07-30.** New unpromoted
      artifact per `07 §Retraining`; trailing 183 days; 90-day recency half-life (DL-047)
      multiplied into the S-601 hypo × confidence weights. ★ `is_promoted` is **not passed**
      to `ModelArtifact` at all — the column defaults False and the S-1001b guard forbids any
      module outside `data.promotion` writing the flag, so the refit *cannot* mint a promoted
      artifact rather than choosing not to. Also delivers the **DB→features assembler that
      had never existed**, `backfill_actual_states` (DL-049), and the `load_shadow_evidence`
      wiring — the operator dashboard shows real numbers for the first time.
      ★ **The adversarial pass found a live hole:** the "drop low-weight old rows" plant
      **survived**, because the positivity guard tested `recency_weights` (the pure function)
      rather than `composite_weights` (what production calls). Two tests added on the
      composition; re-planting now fails them.
